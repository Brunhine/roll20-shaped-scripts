...
class ConfigUi extends ShapedModule {
  ...
  addCommands(commandProcessor) {
    const menus = {
      ...
      amMenu: new AmmoManagerMenu(this.myState.config, ShapedConfig.configOptionsSpec),
    };
...
class ConfigMenu {
  ...
  backToSheetEnhancementsButton() {
    return utils.buildHTML('a', '&lt;-- Sheet Enhancements', {
      href: '!shaped-config --seMenu',
      style: 'text-align: center; margin: 5px 0 0 0; padding: 2px 2px ; border-radius: 10px; white-space: nowrap; ' +
      'overflow: hidden; text-overflow: ellipsis; background-color: #02baf2; border-color: #c0c0c0;',
    });
  }
...
class TokenBarsMenu extends ConfigMenu {
  getMenu() {
    ...
    return table + this.backToTokenOptionsButton();
  }
}
...
class NewCharacterHouseruleMenu extends ConfigMenu {
  getMenu() {
    ...
    return table + this.backToNewCharOptionsButton();
  }
}
...
class SheetEnhancementsMenu extends ConfigMenu {
  getMenu() {
    ...
    const optionRows =
      ...
      this.makeOptionRow({
        title: 'Ammo Manager', path: 'amMenu', command: '', linkText: 'view --&gt;', buttonColor: '#02baf2',
      });

    const th = utils.buildHTML('th', 'Sheet Enhancements', { colspan: '2' });
    const tr = utils.buildHTML('tr', th, { style: 'margin-top: 5px;' });
    const table = utils.buildHTML('table', tr + optionRows, { style: 'width: 100%; font-size: 0.9em;' });
    return table + this.backToMainMenuButton();
  }
...
class AmmoManagerMenu extends ConfigMenu {
  getMenu() {
    const root = 'sheetEnhancements.ammoManager';
    const menu = 'amMenu';

    const optionRows =
      this.makeToggleSetting({
        path: `${root}.enabled`, title: 'Track ammo usage', menuCmd: menu,
      }) +
      this.makeToggleSetting({
        path: `${root}.useTurnCounter`, title: 'Use turn counter', menuCmd: menu,
      }) +
      this.makeInputSetting({
        path: `${root}.recoveryPercent`, title: 'Recovery Percent', prompt: 'Percent recovered', menuCmd: menu,
      }) +
      this.makeToggleSetting({
        path: `${root}.autoRecover`, title: 'Automatically recover ammo', menuCmd: menu,
      });

    const th = utils.buildHTML('th', 'Ammo Manager', { colspan: '2' });
    const tr = utils.buildHTML('tr', th, { style: 'margin-top: 5px;' });
    const table = utils.buildHTML('table', tr + optionRows, { style: 'width: 100%; font-size: 0.9em;' });
    return table + this.backToSheetEnhancementsButton();
  }
}
