/* globals unescape, GroupInitiative */
'use strict';
...
function ShapedScripts(logger, myState, roll20, parser, entityLookup, reporter, srdConverter, sanitise, mpp) {
  ...
  this.handleTurnOrderChange = function handleTurnOrderChange() {
    if (roll20.getCampaign().get('initiativepage')) {
      // turn counter was opened
      if (ammoManager.shouldUseTurnCounter) {
        ammoManager.startAmmoTracking();
      }
    }
    else {
      // turn counter was closed
      if (ammoManager.shouldUseTurnCounter) {
        ammoManager.endAmmoTracking();
      }
    }
  };
  ...
  this.registerEventHandlers = function registerEventHandlers() {
    ...
    roll20.on('change:campaign:initiativepage', this.wrapHandler(this.handleTurnOrderChange));
    ...
    if (typeof GroupInitiative !== 'undefined' && GroupInitiative.ObserveTurnOrderChange) {
      logger.info('Found GroupInitiative. Hooking in to events');
      GroupInitiative.ObserveTurnOrderChange(() => { // eslint-disable-line new-cap
        const before = roll20.getCampaign().get('initiativepage');
        _.delay(() => {
          const after = roll20.getCampaign().get('initiativepage');
